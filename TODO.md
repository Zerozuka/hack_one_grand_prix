...
- [x] `ensure_sos_chat` 関数内で `ChatMessage` を自動挿入している箇所を削除する

#### 動作確認

- [x] スレッドを開いてもチャット欄が空で始まること
- [x] 手動でメッセージを送ると通常通り表示されること
...
- [x] チャット入力欄の `placeholder="返信を入力..."` → `placeholder="コメントを入力"` に変更
- [x] スレッド開閉ボタンのスタイルを変更
...
- [x] placeholder が「コメントを入力」になっていること
- [x] 開いているスレッドのボタンが黒, 閉じているものが青になっていること
...
- [x] チャットメッセージ (`ChatMessage`) の `created_at` を各メッセージの横に表示する
  - フォーマット: `HH:MM` (当日) または `M/D HH:MM` (別日)

#### 動作確認

- [x] 各コメントの横に送信時刻が表示されること
...
- [x] 議論一覧を `is_live === true` のものだけ表示するようにフィルタする
- [x] `is_live === false` の Ended 議論は一覧から除外する (作成ボタン付近に「X 件終了」テキストのみ表示する形でもよい)

#### 動作確認

- [x] Discussion タブに Ended の議論が表示されないこと
- [x] Active の議論のみ一覧に出ること
...
- [x] バックエンド: `POST /v1/events/{id}/end` で `event.creator_user_id != context.user.id` の場合に 403 を返す
- [x] フロントエンド: `event.creator_user_id === currentUserId` の場合のみ End ボタンを表示する

#### 動作確認

- [x] 自分が作成した議論にのみ End ボタンが表示されること
- [x] 他人の議論に End ボタンが表示されないこと
- [x] 他人が API 直接呼び出しで End しようとすると 403 が返ること
...
- [x] バックエンド: `POST /v1/events` で `is_live=true` の場合, 同ユーザーが既に `is_live=true` のイベントを作成済みなら 409 を返す
- [x] フロントエンド: 自分が既にアクティブな議論を持っている場合は作成ボタンを無効化 + 「既にアクティブな議論があります」を表示

#### 動作確認

- [x] 1つ目の議論は作成できること
- [x] 2つ目を作成しようとするとブロックされること
- [x] 1つ目を End してから再作成できること
...
- [x] `activeThread: string | null` → `openThreadIds: Set<string>` に変更
- [x] スレッドのトグルを `openThreadIds.has(id)` で判定するよう変更
- [x] 複数スレッドが同時に展開されていても各スレッドのチャットが独立して表示されるよう修正

#### 動作確認

- [x] 2つ以上のスレッドを同時に開けること
- [x] それぞれのチャット内容が独立して表示されること
- [x] 同じスレッドをもう一度クリックすると閉じること
